...
Здесь происходит следующее: унарный оператор `!` преобразует явно в `boolean` по правилам `ToBoolean`. Итак, еще перед обработкой условие `[] == ![]` уже преобразуется в `[] == false`.
